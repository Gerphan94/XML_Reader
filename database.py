...
class dataConnection(object):
    ...
    def get_chiphi_vtyt(self, ma_lk):
        stm = f"SELECT thanh_tien, t_bntt, t_bhtt, t_bncct FROM xml3 WHERE ma_lk = '{ma_lk}' AND ma_nhom = '10' "
        rows = self.cur.execute(stm).fetchall()
        thanh_tien = 0 
        bntt = 0
        bhtt = 0
    ...
    def get_chiphi_dichvu(self, ma_lk):
        stm = f"SELECT thanh_tien, t_bntt, t_bhtt, t_bncct FROM xml3 WHERE ma_lk = '{ma_lk}' AND ma_nhom != '10'"
        rows = self.cur.execute(stm).fetchall()
        thanh_tien = 0 
        bntt = 0
    ...
    def get_k(self, ma_lk, goi):
        LCS = 1490000
        tranBHYT = LCS*45
        stm = f"SELECT so_luong, don_gia, tyle_tt, t_trantt FROM xml3 WHERE ma_lk = '{ma_lk}' AND goi_vtyt = '{goi}'"
        rows = self.cur.execute(stm).fetchall()
        tong_bhyt = 0
        for row in rows:
            ...
            if (row[3] == ""):
                ...
                t_trantt = float(row[3])

            if (t_trantt == 0.0):
                tong_bhyt = tong_bhyt + (sl*don_gia*ty_le)
            else:
                tong_bhyt = tong_bhyt + (sl*t_trantt*ty_le)
        k = tong_bhyt/tranBHYT
        return k
